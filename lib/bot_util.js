        ...
        if(message.chat.type === 'private') {
            ...
            switch(chat_state) {
                    ...
                    plexmediaserver.checkUsernameHasAccess(message.text.trim()).then(status => {
                        if(status === 'access_granted') {
                            database.users.update('telegram_id', message.from.id, {
                                ...
                                resolve({
                                    response: 'Your Plex.tv username is valid and has been confirmed.',
                                    nextState: 2
                                });
                ...
                case 2: {
                    ...
                    if(cmd === '/makerequest') {
                        resolve({
                            response: 'Send a message using the @imdb bot, or send a link to the content you\'re interested in on imdb.com. It must be one of these two sites, otherwise your request won\'t be accepted. \nExample: https://www.imdb.com/title/tt0213338/',
                            nextState: 3
                        });
                    }
                ...
                case 3: {
                    ...
                    // imdb id from url regex
                    const regex = /tt\d{7}/gm;
